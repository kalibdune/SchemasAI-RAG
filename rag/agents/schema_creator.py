...
class SchemaCreatorAgent:
    def __init__(self, llm: BaseLLM, vector_store: VectorStoreManager):
        system_prompt = """
        ...
        self.creation_prompt = PromptTemplate(
            input_variables=["query", "context"], template=creation_template
        )
        ...
        self.fix_prompt = PromptTemplate(
            input_variables=["schema_json", "errors"], template=fix_template
        )
        self.fix_chain = LLMChain(llm=self.llm, prompt=self.fix_prompt)

    async def process(self, message: dict[str, any]) -> CreatorAnswer:
        user_query = message.get("query", "")

        if not user_query:
            return {"status": "error", "message": "Запрос пустой", "schema": None}

        try:
            retriever_result = (await self.retriever_tool.arun(user_query, k=3)).get(
                "context", ""
            )
            definition_schema = (
                await self.retriever_tool.arun("Описание wf/definition", k=2)
            ).get("context", "")
            ...
            if not is_valid:
                try:
                    ...
                    fixed_json = await self.fix_chain.arun(
                        schema_json=schema_json, errors=errors
                    )
